...
typedef struct Food food[FOODS];
...
void saveScore() { // Spremanje konačnog rezultata igre u tekstualnu datoteku. TRENUTACNO NERADI IDK WHY
	char choice;
	char name[4];
	int i;

	printf("\nDo you want to save your final score? (Y/N): ");
	scanf(" %c", &choice);

	choice = toupper(choice);

	if (choice = 'Y') {
		printf("Write your initials (3 letters): ");

		while (1) {
			scanf("%3s", name);

			if (strlen(name) != 3) {
				printf("Must contain 3 letters! Try again: ");
				continue;
			}
			...
			if (valid) {
				...
				printf("Initials must be all upper case! Try again: ");
			}
		}
		...
		printf("Final score was saved.");
	}
	else {
		printf("Final score was not saved.");
	}
}
